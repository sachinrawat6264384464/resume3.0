...
#DevOps #CloudEngineering #AWS #Kubernetes #CareerGrowth #CloudOpsAI`;
  ...
  return (
    <div className="flex flex-col gap-6 w-full pb-12 text-slate-900 dark:text-slate-100 font-sans relative overflow-x-hidden">
      ...
      {/* LINKEDIN CANDIDATE POSTING FEATURE (REQUIREMENT 12 INTEGRATION) */}
      <div className="bg-white dark:bg-slate-900 rounded-[28px] border border-slate-200 dark:border-slate-800 p-6 shadow-sm flex flex-col gap-4 relative z-10">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight">
                Candidate LinkedIn Post Creator & Publisher
              </h3>
              <p className="text-xs text-slate-500 font-medium">
                Publish your preparation readiness, XP milestones & assessment badges directly to your LinkedIn network.
              </p>
            </div>
          </div>

          <button
            onClick={generateLinkedInTemplate}
            className="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-300 border border-blue-200 dark:border-blue-800 hover:bg-blue-100 transition-colors cursor-pointer"
          >
            ✨ Generate Template
          </button>
        </div>

        {/* CANDIDATE LINKEDIN PROFILE URL INPUT FIELD */}
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center justify-between">
            <span className="flex items-center gap-1.5">
              <Share2 className="w-3.5 h-3.5 text-[#0A66C2]" />
              <span>Candidate LinkedIn Profile URL:</span>
            </span>
            <span className="text-[10px] text-slate-400 font-mono">Saved to PostgreSQL DB</span>
          </label>
          <input
            type="url"
            value={linkedInUrl}
            onChange={(e) => setLinkedInUrl(e.target.value)}
            placeholder="e.g. https://www.linkedin.com/in/sachin-rawat"
            className="w-full p-3 rounded-xl text-xs font-medium bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:outline-none focus:border-[#0A66C2]"
          />
        </div>

        {linkedInSuccessMsg && (
          <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 text-xs font-bold text-emerald-700 dark:text-emerald-300 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2">
            ...
            {linkedInShareUrl && (
              <a
                ...
                className="py-2 px-4 rounded-xl bg-[#0A66C2] hover:bg-[#084e96] text-white text-xs font-black flex items-center gap-1.5 transition-all shadow-sm cursor-pointer shrink-0"
              >
                <span>Complete Post on LinkedIn 🚀</span>
              </a>
            )}
          </div>
        )}
        ...
        <div className="flex flex-col gap-2">
          <label className="text-xs font-bold text-slate-700 dark:text-slate-300">
            Compose LinkedIn Post Update:
          </label>
          <textarea
            rows={4}
            value={linkedInText}
            onChange={(e) => setLinkedInText(e.target.value)}
            placeholder="Click 'Generate Template' above or type your achievement update to share on LinkedIn..."
            className="w-full p-4 rounded-2xl text-xs font-medium bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:outline-none focus:border-[#FF6B00]"
          />
        </div>

        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
          <span className="text-[11px] font-mono text-slate-400">
            Official LinkedIn API Integration • Secure OAuth Token Handling
          </span>

          <button
            onClick={handlePublishLinkedIn}
            disabled={isPublishingLinkedIn}
            className="w-full sm:w-auto px-6 py-3 rounded-xl font-black text-xs text-white bg-[#0A66C2] hover:bg-[#084e96] shadow-md flex items-center justify-center gap-2 transition-all cursor-pointer uppercase tracking-wider disabled:opacity-50"
          >
            {isPublishingLinkedIn ? (
              ...
              <>
                ...
                <span>Publish Post to LinkedIn 🚀</span>
              </>
            )}
          </button>
        </div>
