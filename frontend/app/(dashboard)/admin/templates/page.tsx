...
export default function AdminTemplatesPage() {
  ...
  const handleDeleteStage = async (stage: any) => {
    if (!stage) return;
    if (!confirm(`Are you sure you want to delete stage "${stage.title}"? This will permanently delete the stage and all its questions from the database.`)) {
      return;
    }
    ...
    setTemplates((prevTemplates) => {
      if (!prevTemplates.length) return prevTemplates;
      const updatedStages = prevTemplates[0].stages.filter(
        (s: any) => s.id !== stage.id && s.stage_number !== stage.stage_number
      );
      return [{ ...prevTemplates[0], stages: updatedStages }, ...prevTemplates.slice(1)];
    });
    ...
    try {
      const targetId = stage.id || stage.stage_id || stage.stage_number;
      await apiFetch(`/interviews/stages/${targetId}`, {
        method: "DELETE"
      });
      await loadTemplates();
      showAlert(`Stage "${stage.title}" deleted successfully from Database!`, "success", "Stage Deleted");
    } catch (e: any) {
      showAlert(e?.message || "Failed to delete stage from Database", "error", "Delete Failed");
      await loadTemplates();
    }
  };
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-2.5 shrink-0 self-end md:self-center relative">
                  ...
                  <button
                    onClick={() => handleOpenEditStageModal(stage)}
                    className="px-3 py-2 rounded-xl text-xs font-black text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors flex items-center gap-1.5 cursor-pointer border border-slate-300 dark:border-slate-700 uppercase tracking-wider"
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl text-xs font-black text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/60 hover:bg-rose-100 dark:hover:bg-rose-900/80 transition-colors flex items-center justify-center cursor-pointer border border-rose-200 dark:border-rose-900"
                    title="Delete Stage from Database"
                  >
                    ...
                    {isDropdownOpen && (
                      <div className="absolute right-0 top-full mt-2 w-64 bg-white dark:bg-slate-900 border-2 border-[#FF6B00] rounded-2xl p-2 shadow-2xl z-50 flex flex-col gap-1 text-xs font-bold animate-fadeIn">
                        ...
                        <div className="pt-1 border-t border-slate-100 dark:border-slate-800">
                          <button
                            onClick={() => handleDeleteStage(stage)}
                            className="w-full text-left p-2 rounded-lg bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400 hover:bg-rose-600 hover:text-white transition-colors flex items-center gap-2 font-black uppercase tracking-wider"
                          >
          ...
          <div className="bg-white dark:bg-slate-900 border-2 border-[#FF6B00] rounded-[32px] p-6 sm:p-8 max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl flex flex-col gap-5 text-slate-900 dark:text-white relative">
            ...
            <div className="flex items-center justify-between gap-3 pt-3 border-t border-slate-200 dark:border-slate-800">
              <button
                ...
                className="px-4 py-2.5 rounded-xl text-xs font-black text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/60 hover:bg-rose-600 hover:text-white transition-colors flex items-center gap-1.5 cursor-pointer uppercase tracking-wider border border-rose-200 dark:border-rose-900"
              >
                <Trash2 className="w-4 h-4" />
                <span>Delete Stage</span>
              </button>
