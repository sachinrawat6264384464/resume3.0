...
export default function AdminLoginPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-slate-950 text-slate-100 flex flex-col justify-between font-sans relative overflow-hidden select-none">
      ...
      <header className="relative z-10 w-full px-6 py-5 flex items-center justify-between max-w-7xl mx-auto">
        <Link href="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-2xl bg-[#0B1E36] border-2 border-[#FF6B00]/50 flex items-center justify-center text-[#FF6B00] shadow-lg shadow-[#FF6B00]/20 group-hover:scale-105 transition-transform">
        ...
        <div className="flex items-center gap-3">
          <Link 
            href="/login"
            className="px-4 py-2 rounded-xl bg-slate-900 border border-slate-800 text-xs font-bold text-slate-300 hover:text-white hover:border-[#FF6B00]/50 transition-all flex items-center gap-1.5"
          >
      ...
      <main className="relative z-10 flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md p-8 sm:p-10 rounded-[36px] bg-slate-900/90 border-2 border-[#FF6B00]/40 shadow-2xl backdrop-blur-2xl flex flex-col gap-6 relative">
          
          <div className="absolute top-0 inset-x-0 h-2 bg-gradient-to-r from-[#FF6B00] via-amber-500 to-orange-500 rounded-t-[36px]" />

          {/* Admin Header */}
          <div className="flex flex-col items-center text-center gap-2">
            <div className="w-14 h-14 rounded-2xl bg-[#0B1E36] border-2 border-[#FF6B00]/40 flex items-center justify-center text-[#FF6B00] shadow-lg shadow-[#FF6B00]/20 mb-1">
              <ShieldCheck className="w-7 h-7" />
            </div>

            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[#FF6B00]/10 border border-[#FF6B00]/30 text-[#FF6B00] text-[10.5px] font-mono font-black uppercase tracking-wider">
          ...
          {error && (
            <div className="p-3.5 rounded-2xl bg-rose-950/80 border border-rose-800 text-xs font-bold text-rose-300 flex items-center gap-2">
          ...
          <form onSubmit={handleAdminLogin} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className="w-full px-4 py-3.5 rounded-2xl bg-slate-950 border-2 border-slate-800 text-xs font-mono font-bold text-white focus:outline-none focus:border-[#FF6B00] transition-colors"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-4 rounded-2xl font-black text-xs text-white bg-[#FF6B00] hover:bg-[#e05e00] shadow-lg shadow-[#FF6B00]/30 flex items-center justify-center gap-2 transition-all cursor-pointer uppercase tracking-wider mt-2 disabled:opacity-50"
            >
              {isLoading ? (
                <>
          ...
          <div className="pt-4 border-t border-slate-800/80 flex flex-col gap-2">
            <button
              ...
              disabled={isLoading}
              className="w-full py-3 rounded-2xl font-black text-xs text-amber-400 bg-amber-950/40 border border-amber-800/60 hover:bg-amber-950/80 flex items-center justify-center gap-2 transition-all cursor-pointer"
            >
